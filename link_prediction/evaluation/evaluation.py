...
class Evaluator:
    ...
    def _write_output(self, samples, ranks, predictions):
        ...
        data_folder_file = os.path.join(DATA_PATH, self.dataset.name, f"{self.model.name}_filtered_ranks.csv")
        print(data_folder_file)
        with open(data_folder_file, "w") as output_file:
            output_file.writelines(result_lines)

        filename1 = self.output_prefix + "_4_filtered_ranks.csv"
        print(filename1)
        with open(filename1, "w") as output_file:
            output_file.writelines(result_lines)

        filename2 = self.output_prefix + "_5_filtered_details.csv"
        print(filename2)
