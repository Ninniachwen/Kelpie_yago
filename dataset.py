...
FR_Reduced_2K_women="FR_Reduced_2K_women"
FR_Reduced_2K_no_predecessor="FR_Reduced_2K_no_predecessor"
FR_Reduced_2K_selective="FR_Reduced_2K_selective"

ALL_DATASET_NAMES = [FB15K, FB15K_237, WN18, WN18RR, YAGO3_10, FR_ADELA_3K, FR_EP_13K, FR_FULL_12K, FR_Reduced_2K, FR_Reduced_2K_women, FR_Reduced_2K_no_predecessor, FR_Reduced_2K_selective]
...
class Dataset:
    ...
    def sample_to_fact(self, sample_to_convert: tuple[int, int, int] | tuple[tuple[int, int, int]]) -> tuple[str, str, str]|list[str]:
        if ((type(sample_to_convert[0]) == numpy.int64) | (type(sample_to_convert[0]) == int)) & (len(sample_to_convert) == 3):
            head_id, rel_id, tail_id = sample_to_convert
            result = self.entity_id_2_name[head_id], self.relation_id_2_name[rel_id], self.entity_id_2_name[tail_id]
        else:
            result = []
            #result = "<"
    ...
    def fact_to_sample(self, fact_to_convert: tuple[str, str, str]):
        assert len(fact_to_convert) == 3, f"fact_to_convert does not have length 3: {fact_to_convert}"
        
        head_name, rel_name, tail_name = fact_to_convert
        return self.entity_name_2_id[head_name.strip()], self.relation_name_2_id[rel_name.strip()], self.entity_name_2_id[tail_name.strip()]
